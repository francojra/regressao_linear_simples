...
dados %>%
  summarise(mean(Girth), mean(Height), mean(Volume), # Cálculos da média
            sd(Girth), sd(Height), sd(Volume), # Cálculos do desvio-padrão
            seG = sd(Girth) / sqrt(length(Girth)), # Cálculos do erro-padrão
